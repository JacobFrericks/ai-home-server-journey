...
- [Giving the Server Eyes: A Local Prometheus + Loki + Grafana Stack](posts/2026-07-10-observability-stack/README.md) - Building a self-hosted observability stack as a separate Compose project, with only Grafana on the LAN, scraping host-networked services via `host.docker.internal`, Home Assistant's native Prometheus integration, the uid-65534 token-permission gotcha, and dashboards for the shared 3090 (Ollama vs Plex) and the Plex library
- [Giving the Local AI Internet Access: Self-Hosted SearXNG over MCP](posts/2026-07-11-local-ai-web-access/README.md) - Letting the local LLM and the Home Assistant voice assistant search the web through a self-hosted SearXNG metasearch engine exposed as MCP tools — one privacy-preserving path for both surfaces, the loopback-only wiring, the `mcp-<entry_id>` LLM-API gotcha in HA, and teaching the model when to reach for it
- [From Dashboards to Pings: Getting Server Alerts on My Phone](posts/2026-07-11-alerts-to-phone/README.md) - Wiring Grafana's previously notify-nobody alerts to my phone by routing them through a Home Assistant webhook to the Companion app — the critical-few alerts, provisioned as files, and self-hosted right up to the final push
